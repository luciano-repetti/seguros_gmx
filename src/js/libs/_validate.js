		...
		init = function() {
			...
			$('body').on('focusin', fields, function(e) {
				...
				if (that.attr('placeholder') == undefined) {
					var valueD = that.val();
					that.attr('placeholder', valueD);
				}
				if (that.val() == that.attr('placeholder') && that.attr('readonly') == undefined) {
					that.val('');
				}
			});
		...
		addMessage = function(item, classe) {
			...
			if (!item.parent().hasClass('hintTip')) {
				...
				if (item.parents('.currentInput').length) {
					item.parents('.currentInput').next('.o-msj').addClass('is-visible');
				}

				console.info('pasa validador');

				/* PROYECTO: MAS SALUD */
				var fechaIco = item.parents('form').find('.hasDatepicker'),
					icoCal;
				for (let d = 0; d < fechaIco.length; d++) {
					...
					_validateFields = function(item, classes) {
						...
						if (valueType == 'select') {
							...
							if (item.val() != '') {
								...
								if (item.parents('.currentInput').length) {
									item.parents('.currentInput').next('.o-msj').removeClass('is-visible');
								}
								return false;
							} else {
						...
						if (item.hasClass('required') == true && item.val() == item.attr('placeholder')) {
